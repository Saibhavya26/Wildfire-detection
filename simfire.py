...
# === CONFIGURATION ===
VIDEO_PATH = "fire_video.mp4"   # your drone video file
GRID_SIZE = 20                   # grid size (20x20)
SPREAD_PROB = 0.3                # probability fire spreads to neighbor
SPREAD_INTERVAL = 5              # every N frames simulate spread
THRESHOLD = 180                  # intensity threshold to detect fire
...
while cap.isOpened():
    ...
    if len(burning_cells) > 0:
        ...
        if prev_centroid is not None:
            dx = centroid[1] - prev_centroid[1]  # column shift
            dy = centroid[0] - prev_centroid[0]  # row shift
            direction = ""
            if abs(dx) > 1:
